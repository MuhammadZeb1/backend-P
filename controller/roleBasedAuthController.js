...
//register
 export const register = async (req,res)=>{
    try {
       ...
       if(!name || !email|| !password ||!role ) return res.status(400).json({status:"failed",message:"All fields are required"});

       const existingUser =await User.findOne({email})
       console.log(existingUser)
       if (existingUser) return res.status(400).json({status:"failed",message:"User already exists"});
